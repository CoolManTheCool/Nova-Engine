...
Device::Device(Window& window, const EngineConfig& c) {

    enableValidationLayers = c.debug;
...
void Device::createInstance(const EngineConfig& c) {
    ...
    appInfo.pApplicationName   = c.title.c_str();
    appInfo.applicationVersion = VK_MAKE_API_VERSION(0, c.appMajorVer, c.appMinorVer, c.appPatchVer);
    appInfo.pEngineName        = c.engineName.c_str();
    appInfo.engineVersion      = VK_MAKE_API_VERSION(0, c.engMajorVer, c.engMinorVer, c.engPatchVer);
...
void Device::pickPhysicalDevice(const EngineConfig& c) {
